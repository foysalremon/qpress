...
const PostEdit = ({ medias, post, categories, tags }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setError({ state: 'info', message: 'Be patient. Updating post' });
    const { data } = await axios.patch(`/api/posts/${post._id}`, formData);
    if (data?.state !== 'error') {
      setError({
        state: 'success',
  ...
  return (
    <>
      <Head>
        <title>Edit Post - Q Press</title>
      </Head>
      <Admin active="Posts" subactive="Add Post" title="Edit Post">
        <Card>
...
export async function getServerSideProps(ctx) {
  ...
  const post = await axios
    .get(`${process.env.NEXTAUTH_URL}/api/posts/${ctx.query.id}`)
    .then((r) => r.data);
